        ...
        protected override async Task<Document> GetChangedDocumentAsync(CancellationToken cancellationToken)
        {
            ...
            return await (
                syntaxNode is MethodDeclarationSyntax methodDeclarationSyntax
                    ? (IAsyncMaker)new MethodDeclarationAsyncMaker(methodDeclarationSyntax)
                    : new LocalFunctionStatementAsyncMaker((LocalFunctionStatementSyntax)syntaxNode))
                .GetChangedDocumentAsync(document, root, cancellationToken);
        }

        private interface IAsyncMaker
        {
            Task<Document> GetChangedDocumentAsync(
                Document document, SyntaxNode root, CancellationToken cancellationToken);
        }

        private abstract class AsyncMaker<TSyntax> : IAsyncMaker where TSyntax : SyntaxNode
        {
            protected abstract TSyntax Syntax { get; }

            protected abstract TSyntax AddAsyncModifier();

            protected abstract BlockSyntax? GetBody(TSyntax syntax);

            protected abstract ISymbol? GetDeclaredSymbol(
                SemanticModel semanticModel, CancellationToken cancellationToken);

            protected abstract SyntaxNode WithBody(TSyntax declaration, BlockSyntax? blockSyntax);

            public async Task<Document> GetChangedDocumentAsync(
                Document document, SyntaxNode root, CancellationToken cancellationToken)
            {
                var newDeclaration = AddAsyncModifier();
                var semanticModel = (await document.GetSemanticModelAsync(cancellationToken))!;
                var taskClassSymbol = semanticModel.Compilation.GetTaskClassType()?.OriginalDefinition;
                var body = GetBody(newDeclaration)!;
                MakeMethodAsyncFixer fixer = ((IMethodSymbol?)GetDeclaredSymbol(semanticModel, cancellationToken) ?? throw new InvalidOperationException("Unable to get method symbol."))
                ...
                return document.WithSyntaxRoot(root.ReplaceNode(
                    Syntax,
                    WithBody(newDeclaration, SyntaxFactory.Block(
                        body.AttributeLists,
                        body.OpenBraceToken,
                        [.. fixer.FixedSyntaxes(GetBody(Syntax)!.Statements, needToAppendReturn: false)],
                ...
                SemanticModel semanticModel, INamedTypeSymbol? taskClassSymbol, CancellationToken cancellationToken)
            {
                ...
                private BlockSyntax FixedBlockSyntax(BlockSyntax blockSyntax, bool needToAppendReturn)
                    => SyntaxFactory.Block(
                        ...
                        [.. FixedSyntaxes(blockSyntax.Statements, needToAppendReturn)],
                ...
                private TryStatementSyntax FixedTryStatementSyntax(
                    ...
                    => SyntaxFactory.TryStatement(
                        ...
                        [.. tryStatementSyntax.Catches.Select(catchClauseSyntax => SyntaxFactory.CatchClause(
                ...
                private ElseClauseSyntax? FixedElseClauseSyntax(
                    ElseClauseSyntax? elseClauseSyntax, bool needToAppendReturn)
                ...
                private ForEachStatementSyntax FixedForEachStatementSyntax(
                ...
                private StatementSyntax FixedSubStatementSyntax(
                    StatementSyntax statementSyntax, bool needToAppendReturn)
                {
                ...
                private SwitchStatementSyntax FixedSwitchStatementSyntax(SwitchStatementSyntax switchStatementSyntax)
                    => SyntaxFactory.SwitchStatement(
                        ...
                        [.. switchStatementSyntax.Sections.Select(
                            switchSectionSyntax => SyntaxFactory.SwitchSection(
                                switchSectionSyntax.Labels,
                                [.. FixedSyntaxes(switchSectionSyntax.Statements, needToAppendReturn: true)]))],
            ...
            protected static SyntaxToken BuildAsyncSyntaxToken() => SyntaxFactory.Token(SyntaxKind.AsyncKeyword);
        }

        private sealed class MethodDeclarationAsyncMaker(MethodDeclarationSyntax methodDeclarationSyntax)
            : AsyncMaker<MethodDeclarationSyntax>
        {
            protected override MethodDeclarationSyntax Syntax => methodDeclarationSyntax;

            protected override MethodDeclarationSyntax AddAsyncModifier()
                => methodDeclarationSyntax.AddModifiers(BuildAsyncSyntaxToken());

            protected override BlockSyntax? GetBody(MethodDeclarationSyntax syntax) => syntax.Body;

            protected override ISymbol? GetDeclaredSymbol(
                SemanticModel semanticModel, CancellationToken cancellationToken)
                => semanticModel.GetDeclaredSymbol(methodDeclarationSyntax, cancellationToken);

            protected override SyntaxNode WithBody(MethodDeclarationSyntax declaration, BlockSyntax? blockSyntax)
                => declaration.WithBody(blockSyntax);
        }

        private sealed class LocalFunctionStatementAsyncMaker(LocalFunctionStatementSyntax localFunctionStatementSyntax)
            : AsyncMaker<LocalFunctionStatementSyntax>
        {
            protected override LocalFunctionStatementSyntax Syntax => localFunctionStatementSyntax;

            protected override LocalFunctionStatementSyntax AddAsyncModifier()
                => localFunctionStatementSyntax.AddModifiers(BuildAsyncSyntaxToken());

            protected override BlockSyntax? GetBody(LocalFunctionStatementSyntax syntax) => syntax.Body;

            protected override ISymbol? GetDeclaredSymbol(
                SemanticModel semanticModel, CancellationToken cancellationToken)
                => semanticModel.GetDeclaredSymbol(localFunctionStatementSyntax, cancellationToken);

            protected override SyntaxNode WithBody(LocalFunctionStatementSyntax declaration, BlockSyntax? blockSyntax)
                => declaration.WithBody(blockSyntax);
        }
    }
}
